        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if idx < AGG_LAYER:
                    ...
                    # ---- 论文式打分：每个图像 token 从「所有 query」那里接收到的平均注意力 ----
                    # last_layer_attention: [batch, heads, query_len, key_len]
                    # 1) 对 heads 维平均 → [batch, query_len, key_len]
                    attn_head_avg = torch.mean(last_layer_attention, dim=1)

                    # 图像占多少个 token
                    n_img_cfg = IMAGE_TOKEN_LENGTH
                    n_img_rt = getattr(self, "_fastv_runtime_n_image", None)
                    n_img = int(n_img_rt) if n_img_rt is not None else int(n_img_cfg)
                    ...
                    # 2) 取出图像列：attn[:, :, img_start:img_end] → [batch, query_len, n_img]
                    #    即「每个 query 对每个图像 token」的注意力
                    img_cols = attn_head_avg[:, :, SYS_LENGTH:img_end]

                    # 3) 对 query 维（dim=1）平均：每个图像 token j 从所有 query 那里收到的平均 attention
                    #    → [batch, n_img]
                    img_scores = img_cols.mean(dim=1)

                    # 4) 保留 topk
                    k_keep = min(ATTENTION_RANK, img_scores.shape[-1])
                    k_keep = max(k_keep, 1)

                    # topk 在 n_img 维度上选 → indices 是图像段内局部索引，加 SYS_LENGTH 映射回全局
                    top_attention_rank_index = (
                        img_scores[0].topk(k_keep, dim=-1).indices + SYS_LENGTH
                    )
