...
"""LlamaModel + FastV inplace 剪枝，视觉 token 排名使用双差分（``fastv_diff``）。
...
from .fastv_diff import fastv_image_topk_indices
...
class LlamaModelFastVDiff(LlamaModel):
    """``LlamaModel`` + FastV inplace；剪枝分数见 ``fastv_diff``（可关 ``use_fastv_diff`` 退回原版注意力）。"""

    def __init__(self, config):
        super().__init__(config)
        # Populated by ``configure_llama_fastv_diff`` after load; all off by default.
        ...
        self.use_fastv_diff = True
        self.fastv_diff_lambda_tt = 1.0
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if idx < AGG_LAYER:
                    ...
                    top_attention_rank_index = fastv_image_topk_indices(
                        ...
                        n_img,
                        ATTENTION_RANK,
                        use_diff=bool(getattr(self, "use_fastv_diff", True)),
...
def configure_llama_fastv_diff(
    llama_inner,
    *,
    fastv_k: int = 2,
    fastv_r: float = 0.5,
    ...
    use_fastv_diff: bool = True,
    fastv_diff_lambda_tt: float = 1.0,
) -> None:
    """Attach FastV + diff hyper-parameters to the inner LLaMA (``LlamaModelFastVDiff`` instance)."""
    if fastv_k <= 0:
    ...
    llama_inner.use_fastv_diff = bool(use_fastv_diff)
    llama_inner.fastv_diff_lambda_tt = float(fastv_diff_lambda_tt)
