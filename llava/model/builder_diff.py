...
#
#    Standalone builder for FastV + diff-scoring LLaVA (see llava_llama_diff.py).
...
from llava.model.language_model.llava_llama_diff import LlavaLlamaForCausalLMFastVDiff
from llava.model.language_model.llama_model_diff import configure_llama_fastv_diff
from llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN


def load_pretrained_model_diff(
    model_path,
    model_base,
    model_name,
    load_8bit=False,
    ...
    use_fastv_diff=True,
    fastv_diff_lambda_tt=1.0,
    **kwargs,
):
    """Same as ``load_pretrained_model_fastv`` but uses ``LlavaLlamaForCausalLMFastVDiff`` (diff token scoring)."""
    ...
    if "llava" in model_name.lower():
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            model = LlavaLlamaForCausalLMFastVDiff.from_pretrained(
            ...
            if "mpt" in model_name.lower():
                ...
                model = LlavaLlamaForCausalLMFastVDiff.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
    else:
    ...
    if "llava" in model_name.lower():
        ...
        if "mpt" not in model_name.lower():
            configure_llama_fastv_diff(
                ...
                use_fastv_diff=use_fastv_diff,
                fastv_diff_lambda_tt=fastv_diff_lambda_tt,
            )
            print(
                f"[FastV+diff] enabled: k={fastv_k}, r={fastv_r}, sys_len={fastv_sys_len}, "
                ...
                f"use_diff={use_fastv_diff}, lambda_tt={fastv_diff_lambda_tt}"
            )
